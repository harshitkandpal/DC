...
# Insight: "This shows our current business balance. If one class is too low, we are losing visibility on that customer segment."
...
# Insight: "Identifies which factors drive our target. If two features are highly correlated, we might cut costs by only tracking one."
if len(num_cols) > 1:
...
# VIZ 3: Univariate Analysis (Boxplot for Outliers/Spread)
# Insight: "Shows the median, spread, and extreme outliers of our primary numerical metric. Helps in setting baseline expectations."
if len(num_cols) > 0:
    plt.figure(figsize=(6, 4))
    sns.boxplot(data=df, y=num_cols[0], color="lightblue")
    plt.title(f"Univariate Analysis: Spread of {num_cols[0]}")
    plt.ylabel(num_cols[0])
    plt.show()

# VIZ 4: Multivariate Analysis (Scatter plot colored by Target)
# Insight: "Allows us to see how two key metrics interact and whether distinct clusters form based on our target objective."
if len(num_cols) >= 2:
    plt.figure(figsize=(8, 5))
    sns.scatterplot(
        data=df,
...
# VIZ 5: Time-Based Analysis (Trend over time)
# Insight: "Visualizes macro-trends, seasonality, or cyclical drops in our core metric across historical timelines."
if DATE_COL is not None and TS_TARGET in df.columns:
    plt.figure(figsize=(10, 5))
    # Create a temporary datetime index just for clean plotting
    temp_time_df = df.copy()
    temp_time_df[DATE_COL] = pd.to_datetime(temp_time_df[DATE_COL])

    # Grouping by month to make the chart readable (avoids massive spikes from daily data)
